 ...
 * Names follow the direction of the line through the cell, NOT the visual
 * position of the corner within a box (that naming bit me once already).
 *
 *   CH_UL = ACS_ULCORNER = ┌ (strokes go DOWN and RIGHT from the cell)
 *   CH_UR = ACS_URCORNER = ┐ (strokes go DOWN and LEFT)
 *   CH_LL = ACS_LLCORNER = └ (strokes go UP and RIGHT)
 *   CH_LR = ACS_LRCORNER = ┘ (strokes go UP and LEFT)
 */
enum {
    CH_NONE = 0,
    CH_H = 1,
...
static unsigned int glyph_cp(unsigned char g) {
    ...
    case CH_V:
        return 0x2502; /* │ */
    case CH_UL:
        return 0x250C; /* ┌ ACS_ULCORNER */
    case CH_UR:
        return 0x2510; /* ┐ ACS_URCORNER */
    case CH_LL:
        return 0x2514; /* └ ACS_LLCORNER */
    case CH_LR:
        return 0x2518; /* ┘ ACS_LRCORNER */
    default:
        return ' ';
    }
}
...
/*
 * Direct port of nvtop nvtop_line_plot() per-metric loop (src/plot.c, GPLv3).
 * Critical: NEVER paint back at previous columns. Each column's glyph is
 * placed exactly once by the iteration that owns that column. The previous
 * iteration already drew a horizontal or a corner there; overpainting it
 * with `─` destroys the corner and visually disconnects the line.
 */
static void plot_series_nvtop(Canvas *c, int *vals, int width, int max_y, int col_id) {
    ...
    for (int col = 0; col < width; col++) {
        ...
        /* Bridge skipped columns (only if we had -1 padding between samples). */
        if (col > last_col + 1)
...
int main(void) {
    ...
    /* legend row inside box: working count anchored left, peak count flush
     * right against the box edge. Padding spaces sit in the middle so no
     * dead space trails on the right.
     *
     * visible widths:
     *   left  = ylab_w + 1 (┤) + 1 (sp) + 2 (──) + 9 (' working ') + digits(now)
     *   right = 2 (──) + 6 (' peak ') + digits(max_y)
     */
    char now_buf[16], peak_buf[16];
    int now_len = snprintf(now_buf, sizeof now_buf, "%d", now);
    int peak_len = snprintf(peak_buf, sizeof peak_buf, "%d", max_y);
    int left_vis = ylab_w + 1 + 1 + 2 + 9 + now_len;
    int right_vis = 2 + 6 + peak_len;
    int gap = inner_w - left_vis - right_vis;
    if (gap < 2) gap = 2;
    ...
    ansi(plain, bold);
    ansi(plain, fg220);
    fputs(peak_buf, stdout);
    ansi_reset(plain);
    /* if our gap math overflowed inner_w, fill any remaining cells so the
     * right border still lines up. (Should be zero in practice.) */
    int total_vis = left_vis + gap + right_vis;
    for (int i = total_vis; i < inner_w; i++)
        fputc(' ', stdout);
    ansi(plain, fg141);
    fputc('\xe2', stdout);
    fputc('\x94', stdout);
